    ...
    for (pos.z = 0; pos.z < dims.z - 1; ++pos.z) {
        for (pos.y = 0; pos.y < dims.y - 1; ++pos.y) {
            for (pos.x = 0; pos.x < dims.x - 1; ++pos.x) {
                ...
                // TASK 2: create a nested for loop to construct the cell
                Cell c{};
                ...
                for (int z = 0; z < 2; z++) {  // 3 loops, 8 iterations
                    for (int y = 0; y < 2; y++) {
                        for (int x = 0; x < 2; x++) {
                            ...
                            vec3 scaledCellPos = calculateDataPointPos(pos, localPos, dims);
                ...
                for (const Tetrahedra& tetrahedra : tetrahedras) {
                    ...
                    for (size_t index = 0; index < 4; index++) {
                        if (tetrahedra.dataPoints[index].value < iso) {
                            caseId +=
                                (int)glm::pow(2.0, index);  // calculating caseID based on binaries
...
int MarchingTetrahedra::calculateDataPointIndexInCell(ivec3 index3D) {
    ...
    // Maps a 3D index (x, y, z) to a 1D index in the cell (cell has 8 points, index from 0 to 7)
    // Convention: index = x + y * 2 + z * 4 for a 2x2x2 cell
    // index3D: should be {0,1} for x, y, z
    return index3D.x + index3D.y * 2 + index3D.z * 4;
}
...
vec3 MarchingTetrahedra::calculateDataPointPos(size3_t posVolume, ivec3 posCell, ivec3 dims) {
    ...
    // Returns the position of the data point in the volume, scaled between 0 and 1 for each axis.
    // posVolume: position of the cell in the volume (x, y, z)
    // posCell: position of the data point in the cell {0,1}
    // dims: dimensions of the volume
    // Overall position = cell position + local offset, scaled by dims - 1
    vec3 pos = vec3(posVolume) + vec3(posCell);
    vec3 scaledPos = pos / vec3(dims - ivec3(1));
    return scaledPos;
}
...
void MarchingTetrahedra::createTriangle(const MarchingTetrahedra::DataPoint& v0,
                                        ...
                                        const MarchingTetrahedra::DataPoint& v5, float iso,
